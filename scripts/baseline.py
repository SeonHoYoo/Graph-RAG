...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--question_strategy', type=str, choices=["question_only", "triplet_only", "combined"], default ="combined")
    parser.add_argument("--context_strategy", type=str, choices=["doc_only", "triplet_only", "combined"], default="combined")
...
def build_context(
    ...
    if context_strategy == "doc_only":
        return raw_document_context
    if context_strategy == "triplet_only":
...
def main() -> None:
    ...
    for sample in tqdm(samples, desc="Baseline..."):
        ...
        if args.context_strategy in ["doc_only", "combined"]:
            raw_document_context = build_raw_document_context(sample, args.use_gold_graph)
        if args.context_strategy in ["triplet_only", "combined"]:
    ...
    doc_only_tag = "gold" if args.use_gold_graph == 1 else "all"
    triplet_only_tag = "gold" if args.use_gold_doc == 1 else "all"
    output_name_parts = [f"baseline_{input_stem}", args.context_strategy]
    if args.context_strategy == "doc_only":
        output_name_parts.append(doc_only_tag)
    elif args.context_strategy == "triplet_only":
        output_name_parts.append(triplet_only_tag)
    elif args.context_strategy == "combined":
        if args.use_gold_graph != args.use_gold_doc:
            raise ValueError(
                "combined에서는 use_gold_graph와 use_gold_doc가 같아야 합니다. "
                f"(use_gold_graph={args.use_gold_graph}, use_gold_doc={args.use_gold_doc})"
            )
        combined_tag = "gold" if args.use_gold_graph == 1 else "all"
        output_name_parts.append(combined_tag)
