...
Goal:
Fill placeholder entities like (ENT1), (ENT2), ... in <<Target Question Triplets>> using only the provided evidence.

Hard constraints:
1. Keep triplet order and count exactly the same as input.
2. Preserve all non-placeholder text exactly (relations, [SEP], punctuation, casing, spacing).
3. Replace only placeholders matching pattern (ENTN).
4. If evidence is insufficient or ambiguous, leave that placeholder unchanged.
5. Do not invent entities or infer unsupported links.
6. Output format must be exactly:
   - each triplet wrapped in double quotes
   - triplets separated by comma + space: ", "
   - no extra text, no labels, no explanations

Output only the final infilled triplets.
"""
